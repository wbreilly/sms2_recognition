...
duration_isi_retrieval       = 3000/1000;   % 3000
duration_blank               = 1500/1000;

%%


sub_num=input('Enter subject number: ');    % Ask for subject number input first
% practice=input('Select 1 for practice, 0 for task: ');
practice = 0;

%Instructions text
instruct = 'You will now be presented with verbs and asked\n to recall whether you studied it on the scanning day :\n\n';
...
recog_dat = sprintf('s%02d_recognition.dat',sub_num);
recog_mat = sprintf('s%02d_recognition.mat',sub_num);
...
%% INITIZLIAING PSYCHTOOLBOX
    ...
    response = repmat(default_ans,length(sub_stim),1);       % create a nx1 matrix to contain responses for living/nonliving judgments 
    ...
    word_shown = repmat(default_ans,length(sub_stim),1);     % create a nx1 matrix to contain words shown 
    ...
    for i_word = 1:length(sub_stim)
        ...
        while ((GetSecs - stamp_item_retrieval(i_word)) <= duration_item_retrieval)
            if (KeyCode(key_rem)==0 && KeyCode(key_fam)==0 && KeyCode(key_new)==0)
    ...
    % solve accuracy at a later date
       ...
       % sub item key_press response rt
        ...
        word_shown(i_word) = sub_stim(i_word);
        % save mat
        ...
        RECOGNITION.word_shown               = word_shown;
        
        
        save(recog_mat,'RECOGNITION', 'sub_stim');
end % end i_word
